...
from src.pipelines.pipeline_echo_mimic_acc_mp import Audio2VideoPipeline
...
def get_device_map(num_gpus):
    """
    Returns device mapping for multi-GPU setup
    """
    if num_gpus < 2:
        raise ValueError("This implementation requires at least 2 GPUs")
    return {
        "main": torch.device("cuda:0"),
        "worker": torch.device("cuda:1")
    }
...
def main():
    ...
    for ref_image_path in config["test_cases"].keys():
        for audio_path in config["test_cases"][ref_image_path]:
            ...
            print("time to generate frame: ", time.time() - start_time)
